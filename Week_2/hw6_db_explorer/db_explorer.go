...
// NewRow создает новую строку таблицы
...
// NewRecord создает новую запись
...
// GetTableColumns возвращает информацию о полях
func (d *DbScanner) GetTableColumns(table string) (columns []TableColumn, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get columns for table '%s': %s", table, err)
	}
	defer rows.Close()
...
// IsValidValue валидирует тип значения
...
// ResponseError ответ об ошибке сервера
...
// handleRequest перенаправляет запрос на нужный обработчик
...
// PostTableRecordUpdResponse ответ на запрос обновления записи в таблице
type PostTableRecordUpdResponse struct {
	Updated int `json:"updated"`
}

// handlePostTableRecord обработчик запроса обновления записи в таблице
func (e *DbExplorer) handlePostTableRecord(table Table, id int, data TableRecord) (*PostTableRecordUpdResponse, error) {
	...
	return &PostTableRecordUpdResponse{
		Updated: int(updated),
	}, nil
}
